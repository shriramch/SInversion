...
void printFloatArrayFromCuda(const double arr[], int size) {
    double tempResult[size];
    cudaMemcpy(tempResult, arr, sizeof(double) * size, cudaMemcpyDeviceToHost);
...
// In our case B should be the Identity matrix
     ...
     */
    if (pivot_on) {
        CUSOLVER_CHECK(cusolverDnDgetrs(cusolverH, CUBLAS_OP_N, m, m, /* nrhs */
...
// Added stuff to check against CPU matrix inversion
    ...
    LAPACKE_dgetri(LAPACK_ROW_MAJOR, n, result, n, ipiv);
