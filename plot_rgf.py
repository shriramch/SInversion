...
file_list = [
    "data_rgf1.r0",
    ["data_rgf2.r0", "data_rgf2.r1"],
    "data_rgf1_cuda.r0",
    ["data_rgf2_cuda.r0", "data_rgf2_cuda.r1"],
]
    ...
    for block_size in block_sizes:
        for i, file in enumerate(file_list):  # different algorithm
            ...
            for matrix_size in matrix_sizes:
                matrix_block = os.path.join(
                    file_path, str(matrix_size) + "_" + str(block_size)
                )
                B = block_size
                N = matrix_size
                if isinstance(file, list):  # 2 ranks rgf2 and rgf2_cuda
                    ...
                    max_df = pd.DataFrame(
                        {
                            "time": df1["time"].combine(
                                df2["time"], lambda x1, x2: max(x1, x2)
                            ),
                        }
                    )
                    df = max_df
                    ...
                    ci = sns.utils.ci(
                        sns.algorithms.bootstrap(
                            df["time"].values / 1000, func=np.median, n_boot=1000
                        )
                    )
                ...
                # rgf1 and rgf1_cuda
                    ...
                    df = pd.read_csv(
                        data_path, delim_whitespace=True, comment="#", usecols=["time"]
                    )
                    df = remove_outliers(df)
                    median = np.median(df["time"].values)
                    data_y.append(median / 1000)
                    ...
                    data_ci_high.append(ci[1])
            sns.lineplot(
                x=matrix_sizes, y=data_y, label=algo_names[i], marker=markers[i]
            )
...
def lineplot_performance():
    for block_size in block_sizes:
        for i, file in enumerate(file_list):  # different algorithm
            ...
            for matrix_size in matrix_sizes:
                ...
                if isinstance(file, list):  # 2 ranks rgf2 and rgf2_cuda
                    ...
                    df = remove_outliers(df)
                    median = np.median(df["time"].values)
                    flops_count, cycle_count = rgf2_flops_cycles(
                        matrix_size, block_size, median
                    )
                    # print(f"In {data_path1}, matrix_size={matrix_size} block size={block_size}, median is {median}")
                ...
                # rgf1 and rgf1_cuda
                    ...
                    median = np.median(df["time"].values)
                    data_y.append(median)
                    flops_count, cycle_count = rgf1_flops_cycles(
                        matrix_size, block_size, median
                    )
                    flops_list.append(flops_count)
                    cycles_list.append(cycle_count)
            ...
            sns.lineplot(
                x=matrix_sizes, y=perf_list, label=algo_names[i], marker=markers[i]
            )
        ...
        plt.title(
            f"performance for different matrix sizes with block size {block_size}"
        )
        # plt.yscale('log')
        plt.xscale("log")
        plt.xticks(matrix_sizes, matrix_sizes)
    ...
    for idx, file in enumerate(file_list):  # different algorithm
        for i, block_size in enumerate(block_sizes):
            ...
            sns.lineplot(
                x=matrix_sizes,
                y=data_y,
                label="blocksize=" + str(block_size),
                marker=markers[i],
            )
            plt.fill_between(matrix_sizes, data_ci_low, data_ci_high, alpha=0.2)
...
def boxplot_performance():
    for block_size in block_sizes:
        for i, file in enumerate(file_list):  # different algorithm
            ...
            plt.ylabel("performance (flops/second)")
            plt.title(
                f"{algo_names[i]}: performance for different matrix sizes with block size {block_size}"
            )
            plt.savefig(
                f"./figures/boxplot_performance_block{block_size}_{algo_names[i]}.png"
            )
            plt.clf()
