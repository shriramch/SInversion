...
void rgf2sided(Matrix &A, Matrix &G, bool sym_mat, bool save_off_diag) {
    ...
    if (processRank == 0) {
        ...
        //     write_array("/Users/evan/SInversion/output0.txt", G.lodiag +
        //     (nblocks_2+i)*blockSize*blockSize, blockSize);
        // }
...
void rgf2sided_upperprocess(Matrix &A, Matrix &G, int nblocks_2, bool sym_mat,
    ...
    // std::cout<<"updiag : "<<sizeof(A.updiag)/sizeof(float)<<std::endl;
    // std::cout<<"mdiag : "<<sizeof(A.mdiag)/sizeof(float)<<std::endl;
    ...
    for (int i = 1; i < nblocks_2; ++i) {
        ...
        // write_array(filepath, G_diagblk_leftprocess + i * blockSize *
        // blockSize, blockSize);
    }

    // std::cout<<"After first for loop\n";

    // printFloatArray(G_diagblk_leftprocess, (nblocks_2 + 1) * blockSize * blockSize);
    ...
    //std::cout << "point : after rcv C++ \n";
    //printFloatArray(G_diagblk_leftprocess, (nblocks_2 + 1) * blockSize * blockSize);

    // write_array(filepath, G_diagblk_leftprocess + nblocks_2 * blockSize *
    // blockSize, blockSize);
    ...
    // printFloatArray(temp_result_1, blockSize * blockSize);
    ...
    A.mmSub(blockSize, temp_result_2, temp_result_4, temp_result_2);
            ...
            //std::cout << "point : 000000 C++ \n";
    //printFloatArray(G_diagblk_leftprocess, (nblocks_2 + 1) * blockSize * blockSize);

    A.invBLAS(blockSize, temp_result_2,
              G_diagblk_leftprocess + (nblocks_2 - 1) * blockSize * blockSize);
    // write_array(filepath, G_diagblk_leftprocess +
    // (nblocks_2-1)*blockSize*blockSize, blockSize);
        //std::cout << "point : after INV C++ \n";
    //printFloatArray(G_diagblk_leftprocess, (nblocks_2 + 1) * blockSize * blockSize);

    // Compute the shared off-diagonal upper block
    // write_array(filepath, G_diagblk_leftprocess +
    // nblocks_2*blockSize*blockSize, blockSize); write_array(filepath,
    // A_lowerblk_leftprocess + (nblocks_2-1)*blockSize*blockSize, blockSize);
    A.mmmBLAS(blockSize,
              G_diagblk_leftprocess + nblocks_2 * blockSize * blockSize,
              A_lowerblk_leftprocess + (nblocks_2 - 1) * blockSize * blockSize,
              temp_result_1);
    ...
    // write_array(filepath, G_lowerblk_leftprocess + (nblocks_2-1) * blockSize
    // * blockSize, blockSize);
    if (sym_mat) {
        // matrix transpose
        A.transposeBLAS(
        ...
        // write_array(filepath, G_upperblk_leftprocess +
        // (nblocks_2-1)*blockSize*blockSize, blockSize);
    } else {
        A.mmmBLAS(
            blockSize,
    ...
    //             std::cout << "point : 000000 C++ \n";
    // printFloatArray(G_diagblk_leftprocess, (nblocks_2 + 1) * blockSize * blockSize);
    //     printFloatArray(G_upperblk_leftprocess, (nblocks_2 ) * blockSize * blockSize);
    //         printFloatArray(G_lowerblk_leftprocess, (nblocks_2 ) * blockSize * blockSize);
    ...
    for (int i = nblocks_2 - 2; i >= 0; i -= 1) {
        ...
        if (save_off_diag) {
            ...
            // write_array(filepath, G_lowerblk_leftprocess +
            ...
            if (sym_mat) {
                ...
                A.mmSub(blockSize, zeros, temp_result_2,
                        G_upperblk_leftprocess + i * blockSize * blockSize);
                // write_array(filepath,
                // G_upperblk_leftprocess+i*blockSize*blockSize, blockSize);
            }
        }
        A.mmmBLAS(blockSize, g_ii,
                  A_upperblk_leftprocess + i * blockSize * blockSize,
        ...
        A.mmAdd(blockSize, g_ii, temp_result_2,
                G_diagblk_leftprocess + i * blockSize * blockSize);
        // write_array(filepath, G_diagblk_leftprocess+i*blockSize*blockSize,
        // blockSize);
        delete[] G_lowerfactor;

    //                     std::cout << "point : 11111 C++ \n";
    // printFloatArray(G_diagblk_leftprocess, (nblocks_2 + 1) * blockSize * blockSize);
    //     printFloatArray(G_upperblk_leftprocess, (nblocks_2 ) * blockSize * blockSize);
    //         printFloatArray(G_lowerblk_leftprocess, (nblocks_2 ) * blockSize * blockSize);
    }
...
void rgf2sided_lowerprocess(Matrix &A, Matrix &G, int nblocks_2, bool sym_mat,
    ...
    float *A_diagblk_rightprocess =
        A.mdiag + nblocks_2 * blockSize *
                      blockSize; // nblocks_2 to nblocks-1(included) blocks
    float *A_upperblk_rightprocess =
        A.updiag +
        (nblocks_2 - 1) * blockSize *
            blockSize; // nblocks_2 -1 to nblocks - 2; note updiag is one
                       // smaller than mdiag; A_diagblk_rightprocess and
                       // A_upperblk_rightprocess is of same size
    ...
    //                         std::cout << "point : 11111 C++ \n";
    // printFloatArray(G_diagblk_rightprocess, (nblocks_2 + 1) * blockSize * blockSize);
    //     printFloatArray(G_upperblk_rightprocess, (nblocks_2 ) * blockSize * blockSize);
    //         printFloatArray(G_lowerblk_rightprocess, (nblocks_2 ) * blockSize * blockSize);
    ...
    for (int i = nblocks_2 - 1; i >= 1; i -= 1) {
        ...
        // write_array(filepath, G_diagblk_rightprocess + i * blockSize *
        // blockSize, blockSize);
    }
    ...
    // write_array(filepath, G_diagblk_rightprocess + (1)*blockSize*blockSize,
    // blockSize);
    ...
    for (int i = 1; i < nblocks_2; ++i) {
        ...
        if (save_off_diag) {
            ...
            // write_array(filepath, G_lowerblk_rightprocess + i * blockSize *
            // blockSize, blockSize);
            if (sym_mat) {
                // matrix transpose
                A.transposeBLAS(
                ...
                // write_array(filepath, G_upperblk_rightprocess + i * blockSize
                // * blockSize, blockSize);
            } else {
                A.mmmBLAS(blockSize,
                          G_diagblk_rightprocess + i * blockSize * blockSize,
        ...
        A.mmAdd(blockSize, g_ii, temp_result_2,
                G_diagblk_rightprocess + (i + 1) * blockSize * blockSize);
        // write_array(filepath, G_diagblk_rightprocess +
        // (i+1)*blockSize*blockSize, blockSize);
    }
    ...
    //     write_array(filepath, G.lodiag + (nblocks_2-1+i)*blockSize*blockSize,
    //     blockSize);
    // }
