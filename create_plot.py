...
for i in range(1, len(data), run_count + 1):
    ...
    for j in range(0, run_count):
        ...
        data_parsed[func_type][func_name][n_val].append(
            (float(run_data[0]), float(run_data[1]))
        )
...
for func_type in data_parsed.keys():
    for run_func in data_parsed[func_type].keys():
        for run_n in data_parsed[func_type][run_func].keys():
            ...
            data_aggregated.setdefault(func_type, []).append(
                (run_n, run_func, median, ci, time_median, time_ci, size_median)
            )
...
for run_func in data_aggregated.keys():
    df = pd.DataFrame(
        data_aggregated[run_func],
        columns=["N", "Impl", "Throughput", "CI", "Time", "Time CI", "Size"],
    )
