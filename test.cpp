...
int parse(Config *config, int argc, const char **argv) {
    ...
    struct argparse_option options[] = {
        ...
        OPT_INTEGER('b', "blockSize", &config->blockSize, "block size", NULL, 0, 0), 
        OPT_INTEGER('n', "numRuns", &config->numRuns, "number of runs", NULL, 0, 0),
        OPT_INTEGER('s', "isSymmetric", &config->isSymmetric, "is symmetric", NULL, 0, 0),
...
int main(int argc, const char* argv[]) {
    ...
    if (config.inputPath != NULL) {
        ...
        int BLOCK_SIZE = config.blockSize; // MATRIX_SIZE should be divisible by this
        ...
        for (const auto& algorithm : algorithms) {
            ...
            algorithm.first(inputMatrix, tempResult, MATRIX_SIZE, BLOCK_SIZE, IS_SYMMETRIC, SAVE_OFF_DIAG);
            if(processRank == 0){
        ...
        printf("Usage (random mode): mpirun -np 2 %s -m <matrixSize> -b <blockSize=2> -n <numRuns=10> -s <isSymmetric=false> -o <saveOffDiag=true>\n", bin_name);
